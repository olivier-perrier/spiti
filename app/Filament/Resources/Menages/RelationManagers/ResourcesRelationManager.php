...
class ResourcesRelationManager extends RelationManager
{
    ...
    protected static ?string $title = 'Ressources du ménage';
        ...
        return $table
            ...
            ->columns([
                TextColumn::make('name')->label('Nom')
                    ->sortable(),
                TextColumn::make('birthday')->label('Date de naissance')
                    ->sortable()
                    ->toggleable(),
                TextColumn::make('lot.residence.name')->label('Résidence')->toggleable()->sortable(),
                TextColumn::make('lot.number')->label('Lot')
                    ->sortable()
                    ->toggleable(),
                TextColumn::make('lot.address.street')->label('Adresse')->toggleable()->sortable()
                    ->sortable()
                    ->toggleable(),
                    ...
                    ->schema([
                        Group::make([
                            ...
                            Select::make('lot')->label('Lot')
                                ->options(fn (Get $get) => Lot::query()->where('residence_id', $get('residence'))->pluck('number', 'id'))
