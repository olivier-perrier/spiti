    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('beneficiaries_count')->label('Bénéficiaires')->counts('beneficiaries')->sortable()->toggleable(),
                TextColumn::make('toc')->label('Taux d\'occupation')->suffix('%')
                    ->getStateUsing(function ($record) {
                        return $record->capacity > 0 ? round(($record->beneficiaries_count / $record->capacity) * 100, 2) : null;
                    })->toggleable()
                    ->sortable(query: function ($query, $direction) {
                        $query->withCount('beneficiaries')->orderByRaw("(beneficiaries_count / capacity) $direction");
                    }),
            ])
            ->filters([
